...
# создание кнопок для бота
        ...
        def start_screen(message):
            ...
            if str(message.from_user.id) in self.us:
        ...
        def func(message):
            ...
            if self.flg and not self.flg_vote_end:
                ...
                if not self.flg_vote_end and str(message.from_user.id) not in self.us:
                    ...
                    if str(message.from_user.id) not in self.us:
                ...
                elif self.flg_vote_end and str(message.from_user.id) not in self.us:
                    self.bot.send_message(message.chat.id,
                                          text="Вы".format(
                                              message.from_user))
        ...
        def callback_worker(call):
            if call.data == "vote":
                ...
                if str(call.message.from_user.id) not in self.us:
                    self.flg_vote_end = True
                    ...
                    file.write(' ' + str(call.message.from_user.id))
                    file.close()
                    with open('users.txt', 'r') as f:
                        self.us = f.read().split()
                    f.close()
